...
uint8_t CliProcessCmd(char *pcData, uint8_t ucBytesReceived, char *pcResult)
{
    ...
    bool bIsCmdReceived      = false;
    bool bIsCmdOk            = false;


    bIsCmdReceived = RxHandler(pcData, ucBytesReceived);

    if ( (true == bIsCmdReceived) && (strlen(m_pcCmd) > 0) )
    {
        ...
        for (ucCount = 0; ucCount < usNumOfCmds; ucCount++)
        {
            if (!strcmp(ppcParameters[COMMAND_OFFSET], m_pCliCmdList[ucCount].pcName))
            {
                ...
                {
                    bIsCmdOk = m_pCliCmdList[ucCount].CliExecuteCmd(&ppcParameters[PARAMETER_OFFSET],
